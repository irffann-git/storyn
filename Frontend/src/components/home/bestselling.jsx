...
function Bestselling() {
  ...
  const bestsellingBooks =
    books?.filter((book) => book.tags?.includes("Bestseller")).slice(0, 10) ?? [];
    ...
    <section className="py-16 bg-[#EDE4CB]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-3xl font-bold text-[#37400B]">Bestselling Books</h2>
          <Link
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5 items-start">
          {/* Promo Card — 2/5 (on the left in this layout) */}
          <div
            className="lg:col-span-2 rounded-xl text-white p-8 flex flex-col justify-between h-100 relative overflow-hidden"
            style={{
              backgroundImage: "url('/promo4.jpg')",
              backgroundSize: "cover",
              backgroundPosition: "center",
            }}
          >
            <Link to={"/books?discount=40"}>
            
            {/* Dark overlay so text is readable */}
<div className="absolute inset-0 rounded-xl bg-gradient-to-b from-black/90 via-black/70 to-black/30" />
            {/* Content above overlay */}
            <div className="relative z-10">
              <p className="text-lg font-medium">Big Sales</p>
              <h2 className="text-6xl font-black mt-4">40% OFF</h2>
              <p className="mt-4 text-white/90">Kids LoveReading Books. </p>
              <p className="text-white/90">it all begins with a great books! </p>
          ...
          <div className="lg:col-span-3 min-w-0">
            <Swiper
              ...
              loop={bestsellingBooks.length > 4}
              slidesPerGroup={1}
              breakpoints={{
